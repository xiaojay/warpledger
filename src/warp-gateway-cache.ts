...
const mapStatus = new Map<string, { latestSortKey:string; stateHash:string; state: unknown; validity: Record<string, boolean> }>();
...
const smartweave = WarpFactory
    .forMainnet({
      ...defaultCacheOptions,
      dbLocation: path.join(volumeDir, 'warp/state')
    },)
    .use(new VM2Plugin())
    .useStateCache(
